...
test('farmTradingConfig enables seller-side trading while keeping human cadence', () => {
  ...
  assert.equal(cfg.useDiscordFloor, true);        // 2026-07-07: цена по реальному рынку из Discord
  assert.equal(cfg.cashoutDiscordDiscountPct, 0.06); // листим на −6% от медианы рынка
  ...
  assert.deepEqual(cfg.rarityFullTarget, { rare: 40, epic: 40, legendary: 30 }); // 2026-07-07: пороги "фулл рарности" — стоп-брид + продажа пройденного тира (высокие = пока ничего не пройдено)
